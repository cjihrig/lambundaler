...
const schema = Joi.object({
  ...
  install: Joi.object().keys({
    ...
    version: Joi.string().valid('nodejs', 'nodejs4.3', 'nodejs6.10').optional().default(lambdaDefaults.runtime).description('AWS runtime')
  ...
  deploy: Joi.object().keys({
    ...
    runtime: Joi.string().valid('nodejs', 'nodejs4.3', 'nodejs6.10').optional().default(lambdaDefaults.runtime).description('AWS runtime'),
...
function maybeInstall (settings, next) {
  ...
  Lambstaller(options, function lambstallCb (err) {
    settings.files = settings.files.concat(Path.join(options.out, 'node_modules'));
    next(err, settings);
  });
}
