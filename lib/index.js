...
const schema = Joi.object({
  ...
  deploy: Joi.object().keys({
    ...
    runtime: Joi.string().valid('nodejs', 'nodejs4.3').optional().default('nodejs4.3').description('AWS runtime'),
    timeout: Joi.number().positive().integer().optional().default(3).description('function execution timeout in seconds'),
    memory: Joi.number().positive().integer().multiple(64).optional().default(128).description('allocated function memory in MB')
